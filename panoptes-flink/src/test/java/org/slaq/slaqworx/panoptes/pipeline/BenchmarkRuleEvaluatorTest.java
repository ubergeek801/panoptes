...
public class BenchmarkRuleEvaluatorTest {
  ...
  public void setup() throws Exception {
    ...
    harness =
        new KeyedTwoInputStreamOperatorTestHarness<>(
            evaluatorOperator,
            ((PortfolioEvent e) ->
                e.getBenchmarkKey() != null ? e.getBenchmarkKey() : e.getPortfolioKey()),
            null,
            TypeInformation.of(PortfolioKey.class));
    harness.setup();
    harness.open();
  }
  ...
  public void testBenchmarkBeforeSecurities() throws Exception {
    ...
    List<RuleKey> ruleKeys = IteratorUtils.toList(rulesState.iterator());
    assertEquals(
        1,
        ruleKeys.size(),
        "number of rules in state should equal number of benchmark-enabled rules from portfolio");
    assertEquals(
        rule1Key, ruleKeys.get(0), "rule from state should have matched WeightedAverageRule");

    // nothing should be emitted yet
    Queue<Object> expectedOutput = new LinkedList<>();
    Queue<Object> output = harness.getOutput();
    TestHarnessUtil.assertOutputEquals(
        "output should not be produced for incomplete benchmark", expectedOutput, output);
    ...
    EvaluationResult expectedEvaluationResult =
        new EvaluationResult(
            rule1Key, Map.of(EvaluationGroup.defaultGroup(), new ValueResult(Threshold.WITHIN, 0)));
    ...
    StreamRecord<RuleEvaluationResult> expectedResult =
        new StreamRecord<>(
            new RuleEvaluationResult(
                evaluationResult.eventId(),
                benchmarkKey,
                null,
                EvaluationSource.BENCHMARK,
                true,
                0d,
                0d,
