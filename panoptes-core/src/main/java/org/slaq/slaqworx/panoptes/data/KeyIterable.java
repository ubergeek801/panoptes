...
/**
 * A {@code Closeable} {@code ResultIterable} that retrieves keys on behalf of a Hazelcast
 * {@code MapLoader} (for use with the {@code loadAllKeys()} method) and performs post-query
 * cleanup. Hazelcast promises to call {@code close()} when it completes iterating through the
 * results, at which time the open transaction is completed and the {@code Jdbi} handle is closed.
 *
 * @author jeremy
 * @param <K>
