...
public class PanoptesCacheConfiguration {
   ...
   * Config}.
   *
   * @param cacheName the name of the map/cache being created
   * @param mapStoreFactory the {@link HazelcastMapStoreFactory} to use to create the {@link
   *     MapStore}, or {@code null} to use no {@link MapStore}
   * @return a {@link MapConfig} configured for the given map
   */
  protected MapConfig createMapConfiguration(
      String cacheName, HazelcastMapStoreFactory mapStoreFactory) {
    NearCacheConfig nearCacheConfig =
        new NearCacheConfig()
            .setName("near-" + cacheName)
   ...
   *
   * @param securityAttributeLoader the {@link SecurityAttributeLoader} used to initialize {@link
   *     SecurityAttribute}s
   * @param eligibilityMapConfig the {@link MapConfig} to use for eligibility data
   * @param portfolioMapConfig the {@link MapConfig} to use for {@link Portfolio} data
   * @param positionMapConfig the {@link MapConfig} to use for {@link Position} data
   * @param securityMapConfig the {@link MapConfig} to use for {@link Security} data
   * @param ruleMapConfig the {@link MapConfig} to use for {@link Rule} data
   * @param serializationConfig the {@link SerializationConfig} to use for the cache
   * @param clusterExecutorConfig the {@link ExecutorConfig} to use for clustered execution
   * @param jetConfig the {@link JetConfig} from which to configure Jet, if used
  ...
  protected Config hazelcastConfig(
      SecurityAttributeLoader securityAttributeLoader,
    ...
    ManagedContext applicationContextInjector =
        instance -> {
    ...
    config
        .addMapConfig(eligibilityMapConfig)
        .addMapConfig(portfolioMapConfig)
        .addMapConfig(positionMapConfig)
        .addMapConfig(securityMapConfig)
        .addMapConfig(ruleMapConfig)
   ...
   *
   * @param hazelcastConfiguration the Hazelcast {@link Config} with which to configure the instance
   * @param meterRegistry the {@link MeterRegistry} with which to register Hazelcast resources for
   *     monitoring
  ...
  protected HazelcastInstance hazelcastInstance(
      Config hazelcastConfiguration, MeterRegistry meterRegistry) {
    HazelcastInstance hazelcastInstance =
        Hazelcast.getOrCreateHazelcastInstance(hazelcastConfiguration);

    HazelcastCacheMetrics.monitor(
        meterRegistry, CacheBootstrap.getEligibilityCache(hazelcastInstance));
    HazelcastCacheMetrics.monitor(
        meterRegistry, CacheBootstrap.getPortfolioCache(hazelcastInstance));
    HazelcastCacheMetrics.monitor(
        meterRegistry, CacheBootstrap.getPositionCache(hazelcastInstance));
    HazelcastCacheMetrics.monitor(meterRegistry, CacheBootstrap.getRuleCache(hazelcastInstance));
    HazelcastCacheMetrics.monitor(
        meterRegistry, CacheBootstrap.getSecurityCache(hazelcastInstance));
   ...
   *
   * @param mapStoreFactory the {@link HazelcastMapStoreFactory} to provide to the configuration;
   *     may be omitted if persistence is not desired (e.g. unit testing)
   * @return a {@link MapConfig}
   */
