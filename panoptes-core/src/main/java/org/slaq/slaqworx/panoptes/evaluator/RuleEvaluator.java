 ...
 * optionally a set of benchmark {@code Position}s). The results are grouped by the {@code
 * EvaluationGroupClassifier} defined by the {@code Rule}; there is always at least one group
...
public class RuleEvaluator implements Callable<EvaluationResult> {
   ...
   * {@code Position}s to collect into a new {@code PositionSupplier}, using a {@code
   * Collection} as
   * an accumulator.
   *
   * @author jeremy
  ...
  private class PositionSupplierCollector implements
     ...
     *     the (possibly {@code null} portfolio market value to specify on the created {@code
     *     PositionSupplier}
     */
   ...
   *     the (possibly {@code null}) proposed {@code Position}s (e.g. by a proposed {@code
   *     Trade} to
   *     be combined with the {@code Portfolio} {@code Position}s in a separate evaluation
   * @param evaluationContext
   *     the context in which the {@code Rule} is to be evaluated
   */
  public RuleEvaluator(Rule rule, PositionSupplier portfolioPositions,
