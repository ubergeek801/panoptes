...
/**
 * A Micronaut {@code Factory} that provides {@code Bean}s related to {@code DataSource}s, {@code
 * EntityManager}s, etc.
 *
 * @author jeremy
 */
...
public class PanoptesDataConfiguration {
  ...
  /**
   * Provides a {@code DataSource} which overrides the normally configured {@code
   * DataSourceConfiguration} with a dummy implementation suitable for test and standalone
   * scenarios, which shouldn't use a {@code DataSource} at all (and the dummy will fail if
   * usage is
   * attempted).
   *
