 ...
 * ConfigurableRule}.
 * <p>
 * Note that in order for deserialization to work, the {@link Rule} class must define the static
 * method:
 *
 * <pre>
...
public class RuleSerializer implements ProtobufSerializer<ConfigurableRule> {
   ...
   *
   * @param id
   *     the ID of the {@link Rule} to be created
   * @param description
   *     the description of the {@link Rule} to be created
   * @param ruleClassName
   *     the name of the Java class implementing the {@link Rule}
   * @param configuration
   *     additional {@link Rule} JSON configuration, or {@code null} if not applicable
   * @param groovyFilter
   *     the {@link Rule} filter expression as Groovy, or {@code null} if not applicable
   * @param classifierClassName
   *     the name of the Java class implementing the classifier, or {@code null} if not applicable
   ...
   */
  public static ConfigurableRule constructRule(String id, String description, String ruleClassName,
      String configuration, String groovyFilter, String classifierClassName,
    ...
    if (classifierType == null) {
      ...
      try {
        ...
        throw new RuntimeException(
            "could not instantiate classifier class " + ruleClassName + " for rule " + id + "(" +
                description + ")", e);
      }
    }
    ...
    try {
      Method fromJsonMethod = ruleType
          .getMethod("fromJson", String.class, RuleKey.class, String.class, String.class,
              EvaluationGroupClassifier.class);
      rule = (ConfigurableRule) fromJsonMethod
          .invoke(null, StringUtils.trimToNull(configuration), new RuleKey(id), description,
              StringUtils.trimToNull(groovyFilter), classifier);
    } catch (Exception e) {
      // TODO throw a better exception
      throw new RuntimeException(
          "could not instantiate rule class " + ruleClassName + " for rule " + id + " (" +
              description + ")", e);
    }
   ...
   *
   * @param rule
   *     the {@link ConfigurableRule} to be converted
   *
   ...
   *
   * @param ruleMsg
   *     the {@link RuleMsg} to be converted
   *
   ...
   *
   * @param <T>
   *     the expected type of the returned {@link Class}
   * @param className
   *     the name of the {@link Class} to resolve
   * @param function
   *     the function that the class serves (for logging purposes)
   * @param ruleId
   *     the rule ID for which the {@link Class} is being instantiated (for logging purposes)
   * @param ruleDescription
   *     the rule description for which the {@link Class} is being instantiated (for logging
   *     purposes)
   *
  ...
  protected static <T> Class<T> resolveClass(String className, String function, String ruleId,
    ...
    try {
      ...
      // TODO throw a better exception
      throw new RuntimeException(
          "could not find " + function + " class " + className + " for rule " + ruleId + "(" +
              ruleDescription + ")");
    }
  }
