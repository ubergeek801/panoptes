 ...
 *
 * <p>In general, the calculations and results of {@link Rule} evaluation can be grouped as
 * specified by an {@link EvaluationGroupClassifier}; for example, results may be grouped by {@link
 * Security} currency by providing an {@link EvaluationGroupClassifier} which maps each currency to
 * a distinct group. The default, if an {@link EvaluationGroupClassifier} is not specified, is to
 * calculate for an entire {@link Portfolio}. Note that a {@link Rule} only specifies how its
 * results should be grouped; the actual tabulation is performed by an evaluator such as {@link
 * PortfolioEvaluator}.
 *
 * <p>Currently, a {@link Rule} may have at most one {@link EvaluationGroupClassifier}, which may
 * also act as a {@link GroupAggregator}.
 *
 * @author jeremy
 */
public interface Rule extends BenchmarkComparable, Keyed<RuleKey> {
   ...
   *
   * <p>{@code evaluate()} wraps all {@link Exception}s (including {@link RuntimeException}s) which
   * occur during evaluation into a suitable {@link ValueResult}. Other {link Throwable}s (i.e.
   * {@link Error}s) are not caught.
   *
   * @param positions the {@link Portfolio} {@link Position}s on which to evaluate the {@link Rule}
   * @param evaluationGroup the {@link EvaluationGroup} being evaluated, or {@code null} for the
   *     default group
