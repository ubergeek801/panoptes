...
public interface TradeEvaluator {
  /**
   * Evaluates a {@code Trade} against the {@code Portfolio}s impacted by its {@code
   * Transaction}s.
   *
   * @param trade
   ...
   *
   * @return a {@code CompletableFuture} {@code TradeEvaluationResult} describing the results
   * of the
   *     evaluation
   *
   ...
   */
  CompletableFuture<TradeEvaluationResult> evaluate(Trade trade,
   ...
   * <p>
   * This method uses a binary search algorithm to attempt to converge on an approximate
   * maximum. It
   * will first attempt to allocate {@code MIN_ALLOCATION}, followed by the full target value,
   * after
   * which it will iterate until:
   * <ul>
   ...
   *
   * @return a {@CompletableFuture} {@Double} representing the (approximate) maximum market
   * value of
   ...
   *     if the {@code Thread} was interrupted during evaluation
   */
  CompletableFuture<Double> evaluateRoom(PortfolioKey portfolioKey,
