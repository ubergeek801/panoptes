...
/**
 * Encapsulates the results of a {@code Trade} evaluation. For a given {@code Portfolio} and {@code
 * Rule}, impacts by the proposed {@code Trade} are recorded by {@code EvaluationGroup}.
 *
 * @author jeremy
 */
public class TradeEvaluationResult implements ProtobufSerializable {
  ...
  /**
   * Records an impact corresponding to the given {@code Portfolio}, {@code Rule} and {@code
   * EvaluationGroup}.
   *
   * @param portfolioKey
   ...
   *     a {@code Pair} consisting of a key identifying the {@code Portfolio} under evaluation,
   *     and
   ...
   *
   * @return a {@code Map} associating a {@code Portfolio} and {@code Rule} with another {@code
   * Map}
   *     associating an individual {@code EvaluationGroup} with its measured impact
   */
   ...
   *
   * @return {@code true} if the evaluation results indicate {@code Trade} compliance, {@code false}
   *     otherwise
   */
