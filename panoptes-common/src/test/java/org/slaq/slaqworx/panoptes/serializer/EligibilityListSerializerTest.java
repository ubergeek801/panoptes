...
public class EligibilityListSerializerTest {
  ...
  public void testSerialization() throws Exception {
    ...
    EligibilityList list =
        new EligibilityList(
            "list", ListType.ISSUER, "test list", Set.of("item 1", "item 2", "item 3"));

    byte[] buffer = serializer.write(list);
    EligibilityList deserialized = serializer.read(buffer);

    assertEquals(
        list.name(), deserialized.name(), "deserialized value should have same name as original");
    assertEquals(
        list.type(), deserialized.type(), "deserialized value should have same type as original");
    assertEquals(
        list.description(),
