                ...
                try {
                    BigDecimal roomMarketValue =
                            BigDecimal.valueOf(tradeEvaluator.evaluateRoom(portfolio.getKey(),
                                    securityKey, tradeMarketValue.doubleValue()).get());
            ...
            roomEvaluatorExecutor
                    .submit(() -> portfolioKeys.parallelStream().forEach(portfolioKey -> {
                        try {
                            ...
                            tradeEvaluator
                                    .evaluateRoom(portfolioKey, securityKey,
                                            tradeMarketValue.doubleValue())
                                    .thenAcceptAsync(r -> ui.access(() -> {
                                        try {
                                            ...
                                            if (roomMarketValue.compareTo(BigDecimal.ZERO) != 0) {
                                                ...
                                                allocations.addComponentAtIndex(allocationIndex++,
                                                        allocationPanel);
