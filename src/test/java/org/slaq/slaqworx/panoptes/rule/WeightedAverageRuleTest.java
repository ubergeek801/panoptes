...
public class WeightedAverageRuleTest {
    ...
    public void testEvaluate() {
        ...
        assertFalse("rule with 102% lower limit should have failed", rule.evaluate(TestUtil.p1,
                TestUtil.p2, new EvaluationContext(null, securityProvider, null)).isPassed());

        rule = new WeightedAverageRule(null, "test", null, TestUtil.moovyRating, 1.015, null, null);
        assertTrue("rule with 101.5% lower limit should have passed", rule.evaluate(TestUtil.p1,
                TestUtil.p2, new EvaluationContext(null, securityProvider, null)).isPassed());
        ...
        assertFalse("rule with 3.9 upper limit should have failed", rule
                .evaluate(TestUtil.p1, null, new EvaluationContext(null, securityProvider, null))
                .isPassed());
